...
function Features() {
  ...
  return (
    ...
    >
      ...
      <h1
        ref={title1Ref}
        className="text-4xl md:text-5xl 2xl:text-7xl font-bold mb-6 fade-in"
      >
        Comprehensive{" "}
      ...
      <p
        ref={title2Ref}
        className="max-w-3xl text-gray-700 text-lg 2xl:text-xl mb-12 fade-in stagger-1"
      >
      ...
      <div
        ref={btnRef}
        className="flex flex-wrap justify-center gap-4 mb-12 fade-in stagger-2"
      >
        {categories.map((category) => (
          <button
            key={category}
            onClick={() => setSelectedCategory(category)}
            className={`px-8 py-3 rounded-full font-semibold text-sm transition-all duration-300 border 
              ${
                selectedCategory === category
                  ? "bg-gradient-to-r from-blue-600 to-purple-600 text-white shadow-lg"
                  : "bg-gradient-to-r from-blue-100 to-purple-100 text-blue-800 border-blue-200 hover:scale-105"
              }`}
          >
            {category}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl w-full">
        {filteredFeatures.map((feature, index) => (
          <div
            ...
            className={` p-8 rounded-2xl bg-gradient-to-br from-gray-50 to-white border border-gray-100 hover:border-blue-200 hover:shadow-xl transition-all duration-300 hover-lift fade-in`}
          >
      ...
      {/* Footer */}
      <div ref={footerRef} className={`text-center mt-16 max-w-190 fade-in-up`}>
